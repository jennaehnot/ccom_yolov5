...
save_path = os.path.join(video_dir, 'bitrate_results')
...
# json save name
json_save_name = 'bitrate_results.json'

# plt save name
plot_save_name = 'iouThresh05_bitrate_results.png' # will be added on to buoy_vid name
    ...
    for i, buoy in enumerate(buoy_vids):
        ...
        for bitrate in bitrates:
            ...
            if bitrate == 'native':
                ...
                vid = os.path.join(video_dir, buoy, f"{buoy}_{bitrate}kbps.mp4" )

            try:
                # open video
                v = cv2.VideoCapture(vid)
                count = 1
                bitrate_output = {} #store results for each frame at this bitrate
                ...
                while v.isOpened():
                    ...
                    if imframe is not None: 
                        # convert to RGB, default cv2 vid is BRG and mode3l needs RGB
                        rgb_frame = imframe[:, :, ::-1]
...
# # load model!
# model = torch.hub.load(yolo_dir, 'custom', path = weights_path, source='local',force_reload=True)
# savefpos_pth = '/home/field/jennaDir/test_vids/falsepos'
# def run_inference():
#     for i, buoy in enumerate(buoy_vids):
#         buoy_inf_output = {}
...
#                                     if correct_bb[0][0] == False: # if bbox is false, meaning IoU <0.05
#                                         match = False
#                                         savefpos = os.path.join(savefpos_pth, buoy, 'bitrate', f'{bitrate}_{d:.2f}m')
#                                         results.save(labels=True,save_dir=savefpos)
...
#             if bitrate == 'native' and q == 'lobsterPot':
#                 axis[i].set_title(f"Bitrate = 7460 kbps (native)," + r'  $\bar{t} = $'+ f"{avg_t:.2f} ms", fontsize = 14)
#             elif bitrate == 'native' and q == 'redBall':
#                 axis[i].set_title(f"Bitrate = 6991 kbps (native)," + r'  $\bar{t} = $'+ f"{avg_t:.2f} ms", fontsize = 14)
#             elif bitrate == 'native' and q == 'mooringBall':
#                 axis[i].set_title(f"Bitrate = 8435 kbps (native)," + r'  $\bar{t} = $'+ f"{avg_t:.2f} ms", fontsize = 14)
#             else:
#                 axis[i].set_title(f"Bitrate = {str(bitrates[i])} kbps," + r'  $\bar{t} = $'+ f"{avg_t:.2f} ms", fontsize = 14)
